...
The property verbs do NOT yet: they are read-modify-write against the store's wholesale `Update`, which the store itself documents as replaying a stale wire snapshot and silently dropping a concurrent peer mutation. In practice `resource set-property` racing the tiebreaker or migration reconciler can revert a flag that reconciler has just stamped, with no error. The store offers `PatchProps`, `PatchNodeSpec`, `PatchStoragePoolSpec`, `PatchResourceGroup`, `PatchResourceDefinitionSpec` and `PatchVolumeDefinitionSpec` for exactly this; moving the property accessors onto them is outstanding work, recorded here rather than claimed as done. An earlier draft of this document described these writes as server-side apply, which was never true of the implementation.
