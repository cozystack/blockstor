...
Modifications must not strip the identities the controller allocated — port, node-id, per-volume minor — because those are exactly the fields whose loss causes a DRBD reconnect or a full resync. The store's carry-across rules on update are the reference for what has to survive a modify, and the per-replica verbs (`toggle-disk`, `activate`, `deactivate`) go through `PatchResourceSpec`, so a concurrent reconciler write cannot be lost.

Every other mutating verb now does too. There is no wholesale `Update` left in the CLI: the property verbs, `drbd-options`, `node evacuate`/`restore`, `resource-definition modify`, `resource-group modify`, `volume-group create`, `volume-definition set-size` and the physical-device attach all go through the store's `Patch*` entry points, which fetch current state, apply the caller's change to it, and retry the whole cycle on conflict.

The distinction that matters is not which method is called but what the caller hands over. A verb that reads an object, edits it locally and writes the result back replaces everything, so a key or flag another writer added in between is reverted — silently, and with exit 0. A verb that hands over a *change* has it applied to whatever the object currently is. The property accessors therefore expose an `edit(change func(map[string]string) error)` rather than a `set(map[string]string)`: the CLI can no longer express "make the bag exactly this", only "put this key in it". `resource set-property` racing the tiebreaker or migration reconciler now keeps both writes.

The same reasoning moves decisions inside the patch. `volume-definition set-size` used to read the size, decide whether the change was a shrink, and then write an absolute value; a grow landing in that window left the decision made against a size that no longer existed and the write truncated a live volume. The check now runs inside the closure, against the state the write lands on, and re-runs if the store retries. `volume-group create` picks the next free volume number inside the patch for the same reason — two concurrent creates against one snapshot both pick the same number.

What must hold regardless of who is writing does not belong in a client at all. The volume size bounds are `Minimum`/`Maximum` on the CRD field, so the API server enforces them for the CLI, the REST layer, a controller and a stray `kubectl apply` alike; the identity fields the controller allocates (`drbdPort`, `drbdNodeID`, `skipInitialSync`) are settable-once CEL rules for the same reason. A check that lives in one client is not a check the data is subject to. The shrink refusal stays in the write path because it is a policy with a deliberate `--force` override rather than an invariant, but it is now evaluated atomically against the state it guards.

An earlier draft of this document described these writes as server-side apply, which was never true of the implementation. They are conflict-retried patches; the safety property is the same, and this is what the code does.
