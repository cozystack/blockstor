...
Migrated: nodes, storage pools, resource groups, resource definitions (+ volume definitions), resources (replicas), and SUCCESSFUL snapshots. Controller-wide `DrbdOptions/*` become a `ControllerConfig`. DRBD minors, per-replica node-ids, the shared-secret and (when captured) TCP ports are preserved verbatim. An RD is stamped `Initialized=true` **only when a data-bearing replica was actually adopted for it**, and every replica gets `skipInitialSync=false`, so any replica added **after** migration performs a real sync instead of falsely coming up UpToDate. A definition whose replicas were all skipped — every one DELETE-flagged, or on a node that did not migrate — arrives unlatched, because latching it would tell the controller that data is present when none was adopted, and the first sync would then be free to overwrite whatever the definition later receives. The report on stderr names each definition left unlatched and why. Snapshots are stamped `blockstor.io/adopted` so the controller records them as complete without re-taking them.
...
| `Initialized` is conditional | A definition with no adopted data-bearing replica arrives unlatched, and auto-primary election stays enabled for it | Read the report: each unlatched definition is named. Expected for definitions whose replicas were all being deleted; investigate any other |
