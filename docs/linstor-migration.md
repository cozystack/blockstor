...
Migrated: nodes, storage pools, resource groups, resource definitions (+ volume definitions), resources (replicas), and SUCCESSFUL snapshots. Controller-wide `DrbdOptions/*` become a `ControllerConfig`. DRBD minors, per-replica node-ids, the shared-secret and (when captured) TCP ports are preserved verbatim. An RD is stamped `Initialized=true` whenever its data still exists somewhere, and every replica gets `skipInitialSync=false`, so any replica added **after** migration performs a real sync instead of falsely coming up UpToDate. The latch is conditional, and it errs towards staying on: a definition is left unlatched **only** when no replica is being migrated at all, which in practice means every one was already DELETE-flagged. A definition whose replicas were held back because their nodes did not migrate keeps the latch, because that data is still on those nodes; so does one whose replicas were skipped for pool divergence. Unlatching in either case would let the controller seed an empty first sync over data that exists. The report on stderr names every definition in these states and says which applies, so do not clear a latch by hand on the strength of a definition arriving without replicas. Snapshots are stamped `blockstor.io/adopted` so the controller records them as complete without re-taking them.
...
| `Initialized` is conditional | Left unlatched only when no replica migrates at all; a definition held back by an unmigrated node or by pool divergence keeps the latch and arrives with no replicas | Read the report, which names each case. Unlatched is expected only where every replica was DELETE-flagged. Do not clear a latch by hand because a definition has no replicas — that is what re-enables the auto-primary election this latch exists to suppress |
