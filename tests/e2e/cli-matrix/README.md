...
| `rd-clone-retry-semantics.sh` | parity | The clone endpoint's retry contract. `linstor rd clone <src> <dst>` run twice answers the same way twice, and still does after `vd set-size` grows the source: a finished clone is a copy of a point-in-time and owes the source nothing, while a guard comparing the internal snapshot to the live source made an ordinary resize refuse every later retry permanently (the snapshot outlives the clone by design, so the refusal's own correction cannot be followed). Also pins a clone body carrying `delete_namespaces`, driven over raw REST because the verb has no flag for it — it is a wire field from golinstor's `GenericPropsModify`, and a 400 before the triple was declared. |
