...
# Quick sanity: both must report Primary. Wait briefly for the
# observer to stamp Status.Role after the second promote — events2
# typically reflects on the order of 100ms but we give it 10s.
wait_role "$RD" "$N1" "Primary" 10 || true
wait_role "$RD" "$N2" "Primary" 10 || true
n1_role=$(status_role "$RD" "$N1")
n2_role=$(status_role "$RD" "$N2")
if [[ "$n1_role" != "Primary" || "$n2_role" != "Primary" ]]; then
...
wait_role "$RD" "$N1" "Secondary" 10 || true
