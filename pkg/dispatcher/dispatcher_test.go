...
// TestBug342TiebreakerPeerRenderedAsDiskless pins the dispatcher
// half of Bug 342 Fix A. Once `ensureTiebreaker` keeps the TIE_BREAKER
// witness alive across a `r d <diskful>` (the controller-side half of
// the fix), the surviving Resource's `runApply` pass still hands the
// witness Resource to `BuildDesired` via `listPeerResources`. The
// dispatcher must render it as a real peer entry — `peer.<witness>.*`
// in DrbdOptions plus a `Peers` entry — and surface the `diskless`
// marker so the satellite `.res` renderer emits `disk none` for the
// witness's `on <node>` block (mirrors upstream LINSTOR's
// `disk /dev/drbd/this/is/not/used` ↔ `disk none` flip across
// diskful↔TieBreaker transitions, v16 baseline).
//
// Without a rendered peer block for the witness the surviving
// node's .res shrinks to single-host shape: no `on <witness>` block,
// no `connection { ... }` block, kernel-side peer slot orphans, the
// next `r c <node>` wedges in Connecting/StandAlone forever.
// Contrast with TestInactivePeerDroppedFromSiblings — INACTIVE
// peers are dropped wholesale; TIE_BREAKER (a DISKLESS variant) is
// kept with `disk none`.
func TestBug342TiebreakerPeerRenderedAsDiskless(t *testing.T) {
	t.Parallel()

	const rdName = "pvc-bug342-dispatcher"
	...
	// Target: the lone surviving diskful on n1 (node-id 0).
	target := newDiskfulResource(rdName, "n1", id(0))

	// Peer: the held-open TIE_BREAKER witness on n3 (node-id 2).
	// Bug 342 Fix A keeps this Resource alive across the
	// `r d <other-diskful>` transition; listPeerResources hands it
	// back through to BuildDesired.
	witness := newDisklessResource(rdName, "n3", id(2))
	witness.Spec.Flags = []string{"DISKLESS", "TIE_BREAKER"}

	got := dispatcher.BuildDesired(target, []blockstoriov1alpha1.Resource{*witness}, nil, nil, rd, nil)
	if got == nil {
		t.Fatalf("BuildDesired returned nil")
	}

	// Peers must contain n3 — the witness IS a peer for .res
	// rendering purposes (DRBD-9 protocol-level voter), it just has
	// no backing disk.
	if len(got.Peers) != 1 {
		t.Fatalf("Peers: got %v, want exactly [n3] (TIE_BREAKER must stay in peer set)", got.Peers)
	}

	if got.Peers[0].Name != "n3" {
		t.Errorf("Peers[0].Name=%q, want %q", got.Peers[0].Name, "n3")
	}

	if got.Peers[0].NodeID != 2 {
		t.Errorf("Peers[0].NodeID=%d, want 2 (witness's allocated DRBD node-id)", got.Peers[0].NodeID)
	}

	// drbdOpts must carry the peer.n3.* entries — without them the
	// .res renderer can't emit the `on n3 { ... }` and
	// `connection { ... }` blocks.
	if _, ok := got.DrbdOptions["peer.n3.node-id"]; !ok {
		t.Errorf("drbdOpts missing peer.n3.node-id — witness peer block must render (drbdOpts=%v)", got.DrbdOptions)
	}

	if _, ok := got.DrbdOptions["peer.n3.port"]; !ok {
		t.Errorf("drbdOpts missing peer.n3.port — witness peer block must render (drbdOpts=%v)", got.DrbdOptions)
	}

	// The diskless marker drives the .res renderer's `disk none`
	// emission for the witness's `on n3 { ... }` block (vs the
	// diskful placeholder `disk /dev/drbd/this/is/not/used;`).
	// Mirrors upstream LINSTOR's flip-style transition across
	// diskful↔TieBreaker (v16 empirical baseline).
	if got.DrbdOptions["peer.n3.diskless"] != "true" {
		t.Errorf("drbdOpts['peer.n3.diskless']=%q, want %q (TIE_BREAKER → `disk none` in rendered .res)",
			got.DrbdOptions["peer.n3.diskless"], "true")
	}
}

// TestBug342DiskfulShrinkRetainsPeerBlock simulates the full
// Phase-2 sequence of `r-full-lifecycle` at the dispatcher level:
// initial state has 2 diskful + 1 TIE_BREAKER, the operator runs
// `r d <one-of-diskful>`, and (Bug 342 Fix A) the witness Resource
// stays alive in the peer set. The surviving Resource's
// `BuildDesired` output MUST keep the witness peer rendered so the
// .res file retains its peer-name continuity through the transition
// — the load-bearing invariant that prevents the kernel-slot orphan
// the diff investigation pinned as the bug's root cause.
func TestBug342DiskfulShrinkRetainsPeerBlock(t *testing.T) {
	t.Parallel()

	const rdName = "pvc-bug342-shrink"
	...
	// Surviving target: n1 diskful (node-id 0).
	target := newDiskfulResource(rdName, "n1", id(0))

	// Peer set after the operator's `r d n2` AND with Bug 342 Fix A
	// keeping the witness: only the witness on n3 remains. Pre-fix,
	// `removeWitnesses` would also have deleted the n3 Resource and
	// `peers` here would be empty — that's the 1-host .res failure
	// mode the fix prevents.
	witness := newDisklessResource(rdName, "n3", id(2))
	witness.Spec.Flags = []string{"DISKLESS", "TIE_BREAKER"}
	...
	// At minimum, the witness must remain in Peers so the .res
	// rendered downstream keeps the `on n3` + `connection`
	// blocks. Pre-fix, dispatcher's input `peers []` would have
	// been empty here (the witness CRD had been physically
	// deleted), producing a single-host .res with no peer blocks
	// — kernel-slot orphans wedged at the next `r c`.
	if len(got.Peers) == 0 {
		t.Fatalf("Bug 342: Peers is empty — surviving target would render a 1-host .res (kernel slot orphan); DrbdOptions=%v",
			got.DrbdOptions)
	}
	...
	if !foundN3 {
		t.Errorf("Bug 342: witness n3 missing from rendered Peers — peer-name set must be preserved across diskful-shrink; got %v",
			got.Peers)
	}
}
