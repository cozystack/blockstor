...
func NewManager(restCfg *rest.Config, cfg Config) (manager.Manager, error) {
	...
	// Bug 318: thread the observer-trigger channel through
...
// unset. Today that's just the Bug 318 ReconcileTrigger channel — a
...
func addBackgroundRunnables(mgr manager.Manager, cfg Config) error {
	err := mgr.Add(&ObserverRunnable{
		...
		// Bug 318: producer end of the observer-trigger channel.
