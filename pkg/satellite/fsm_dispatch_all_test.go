...
// fakePeerForgetStamper records every StampPeerForgetAck call so a
// test can assert the dispatcher routed the ACK after running
// del-peer + forget-peer. 342-v10 Phase 2.
type fakePeerForgetStamper struct {
	calls []fakePeerForgetCall
}

type fakePeerForgetCall struct {
	resourceName string
	peerName     string
}

func (f *fakePeerForgetStamper) StampPeerForgetAck(_ context.Context, resourceName, peerName string) error {
	f.calls = append(f.calls, fakePeerForgetCall{resourceName: resourceName, peerName: peerName})

	return nil
}

// TestDispatchFsmActionForgetPeerRunsDelAndForget pins the 342-v10
// Phase 2 contract: when Observation.PeersDeleting carries at least
// one entry with a resolved NodeID, dispatchFsmAction MUST run
// `drbdadm del-peer` + `drbdmeta forget-peer` per (peer, volume)
// and call StampPeerForgetAck so the REST handler's
// waitForPeerDeletionAcks loop unblocks. A regression that wired
// the action to a no-op would silently re-introduce the stale
// kernel-slot wedge.
func TestDispatchFsmActionForgetPeerRunsDelAndForget(t *testing.T) {
	fx := storage.NewFakeExec()
	stamper := &fakePeerForgetStamper{}
	...
	dr, devices := dispatchFixtureDR("pvc-dispatch-forget")
	...
	err := rec.dispatchFsmAction(context.Background(), dr, devices, ActionForgetPeer, obs)
	if err != nil {
		t.Fatalf("dispatchFsmAction(ActionForgetPeer): %v", err)
	}

	cmds := fx.CommandLines()
	mustHave := []string{
		"drbdadm del-peer n2:pvc-dispatch-forget",
		"drbdmeta --force pvc-dispatch-forget/0 v09 /dev/vg/pvc-dispatch-forget_00000 internal forget-peer 1",
	}
	...
	if len(stamper.calls) != 1 ||
		stamper.calls[0].resourceName != "pvc-dispatch-forget.n1" ||
		stamper.calls[0].peerName != "n2" {
		t.Errorf("StampPeerForgetAck calls = %+v; want one call for (pvc-dispatch-forget.n1, n2)", stamper.calls)
	}
}

// TestDispatchFsmActionForgetPeerSkipsUnresolved pins the defer
// invariant from PeerDeletionIntent.NodeID negative sentinel: a
// peer whose NodeID could not be resolved (collectPeersDeleting
// returned -1) MUST NOT trip del-peer / forget-peer. The v8
// footgun was running del-peer without forget-peer, which left
// per-volume metadata stale; this test pins the defer so the next
// reconcile can retry with a populated id.
func TestDispatchFsmActionForgetPeerSkipsUnresolved(t *testing.T) {
	fx := storage.NewFakeExec()
	stamper := &fakePeerForgetStamper{}
	...
	dr, devices := dispatchFixtureDR("pvc-dispatch-forget-defer")
	...
	err := rec.dispatchFsmAction(context.Background(), dr, devices, ActionForgetPeer, obs)
	if err != nil {
		t.Fatalf("dispatchFsmAction(ActionForgetPeer, unresolved): %v", err)
	}

	for _, line := range fx.CommandLines() {
		if strings.HasPrefix(line, "drbdadm del-peer") || strings.HasPrefix(line, "drbdmeta") {
			t.Errorf("defer invariant broken: peer with NodeID<0 must not trip del-peer / forget-peer; got %q", line)
		}
	}

	if len(stamper.calls) != 0 {
		t.Errorf("StampPeerForgetAck unexpectedly called for an unresolved peer: %+v", stamper.calls)
	}
}

// TestFsmTransitionFiresForgetPeerWhenDeleting pins the FSM table
// edge for 342-v10: PhaseRunning with PeersDeleting non-empty AND
// KernelLoaded must surface ActionForgetPeer ahead of ActionAdjust
// (the default Running self-loop). A regression that re-ordered the
// transition table would silently drop the forgetPeer dispatch and
// reintroduce the Bug 342 wedge.
func TestFsmTransitionFiresForgetPeerWhenDeleting(t *testing.T) {
	obs := Observation{
		SpecHasResource: true,
		ResFileExists:   true,
		MetadataExists:  true,
		KernelLoaded:    true,
		PeersDeleting:   []PeerDeletionIntent{{Name: "n2", NodeID: 1}},
	}

	phase := ObservePhase(obs)
	if phase != PhaseRunning {
		t.Fatalf("ObservePhase = %s; want %s", phase, PhaseRunning)
	}

	next := NextTransition(phase, obs)
	if next == nil {
		t.Fatalf("NextTransition returned nil; want ActionForgetPeer")
	}

	if next.Action != ActionForgetPeer {
		t.Errorf("first matching transition action = %s; want %s (must shadow ActionAdjust)", next.Action, ActionForgetPeer)
	}
}

// TestFsmTransitionForgetPeerSkipsWhenKernelDown pins the
// defense-in-depth guard on the trigger: PeersDeleting non-empty
// but KernelLoaded=false means there's no kernel slot to clean up,
// so the transition MUST fall through to the next matching row
// (which is the default Running self-loop ActionAdjust, but only
// when KernelLoaded — so under !KernelLoaded the FSM emits no
// transition for PhaseRunning at all). Pinning this prevents an
// accidental forget-peer kernel call against a downed slot.
func TestFsmTransitionForgetPeerSkipsWhenKernelDown(t *testing.T) {
	obs := Observation{
		SpecHasResource: true,
		MetadataExists:  true,
		KernelLoaded:    false, // kernel slot absent
		PeersDeleting:   []PeerDeletionIntent{{Name: "n2", NodeID: 1}},
	}

	next := NextTransition(PhaseRunning, obs)
	if next != nil && next.Action == ActionForgetPeer {
		t.Errorf("ActionForgetPeer fired against an unloaded kernel slot; trigger should require KernelLoaded")
	}
}

// TestFsmTransitionAdjustWhenNoDeleting pins the negative case:
// PeersDeleting empty but kernel loaded must still pick
// ActionAdjust (the default Running self-loop). Without this the
// FSM would block adjust convergence after the forget-peer table
// row was added.
func TestFsmTransitionAdjustWhenNoDeleting(t *testing.T) {
	obs := Observation{
		SpecHasResource: true,
		MetadataExists:  true,
		KernelLoaded:    true,
	...
	next := NextTransition(PhaseRunning, obs)
	if next == nil {
		t.Fatalf("NextTransition returned nil; want ActionAdjust")
	}

	if next.Action != ActionAdjust {
		t.Errorf("got action = %s; want %s (PeersDeleting empty → no forget-peer)", next.Action, ActionAdjust)
	}
}

// TestFsmTransitionDecommissionStillWinsOverForgetPeer pins the
// shadow ordering: even when a deletion intent is present AND the
// local Resource has DeletionTimestamp set, the FSM's first row
// (Decommissioning) must fire ahead of ActionForgetPeer. Otherwise
// the local satellite would clean up sibling peer state during its
// own teardown, which the dedicated DeleteResource pipeline already
// owns.
func TestFsmTransitionDecommissionStillWinsOverForgetPeer(t *testing.T) {
	obs := Observation{
		SpecHasResource:   true,
		MetadataExists:    true,
		KernelLoaded:      true,
	...
	next := NextTransition(PhaseRunning, obs)
	if next == nil {
		t.Fatalf("NextTransition returned nil; want ActionDecommission")
	}

	if next.Action != ActionDecommission {
		t.Errorf("got action = %s; want %s (local delete must shadow peer-forget)", next.Action, ActionDecommission)
	}
}
