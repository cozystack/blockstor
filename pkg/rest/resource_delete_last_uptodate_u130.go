...
// resourceMidSyncDeleteRefusal reports whether deleting `target` would
// strand a mid-sync peer per U130. True means the delete must be refused.
//
// The rule itself is validate.MidSyncDeleteRefusal, shared with the CLI. It
// used to be a second implementation here, with its own copy of the DRBD
// state table — two answers to a question that must have one, since the CLI
// writes the same objects this handler does.
//
// What stays here is the HTTP shaping below: which status and envelope the
// refusal takes, and the `?force=true` escape hatch.
func resourceMidSyncDeleteRefusal(target *apiv1.Resource, siblings []apiv1.Resource) bool {
	return validate.MidSyncDeleteRefusal(target, siblings)
}
