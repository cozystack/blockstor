...
func (s *Server) handleResourceDelete(w http.ResponseWriter, r *http.Request) {
	...
	// 342-v10 Phase 1+2+3: 2-phase delete handshake. Stamp
	// `DELETING` on the doomed Resource so siblings' satellite
	// FSM observes it and runs ActionForgetPeer (del-peer +
	// forget-peer + stamp peer-forget ACK annotation). Bump
	// peer-changed siblings BEFORE the physical Delete so the
	// satellites wake to see DELETING. Then waitForPeerDeletionAcks
	// blocks for up to peerDeleteAckTimeout (15s) until every
	// online sibling has stamped the ACK. The pre-Delete bump is
	// the wake; the post-Delete bump (further below) is the final
	// adjust convergence after the peer Resource is gone.
	//
	...
	// been notified.
	if getErr == nil && !slices.Contains(existing.Flags, apiv1.ResourceFlagDeleting) {
