...
- [ ] `tests/e2e/tiebreaker.sh` flakes when run immediately after `two-volume-rd` in the same iter batch (2026-05-12). Direct invocation against a warm controller passes; iter-batch invocation does not. Hypothesis: the freshly-restarted controller in iter's rollout step misses the witness-create reconcile window before the test's 30s deadline expires. Need to either widen the test's wait (60s) or stamp witness creation eagerly on RD apply. Tracked under 8.8 follow-up; controller-side NotFound-tolerance on the Get + Patch in `ensureDRBDIDs` already landed (commits `5448199` + `f94d4b4`) so the race no longer produces hard reconcile errors.
