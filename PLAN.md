...
- [~] Delete the gRPC supervise loops in `pkg/satellite/agent.go` (2026-05-11). `startGRPCServer` is gone; `runObserveLoop` / `superviseObserveLoop` / `runCapacityLoop` / `dial` / `hello` stay because the satellite still pushes ObserveStreamEvent + capacity to the controller. Closes once Status SSA replaces the gRPC observe path.
