...
- [~] Reconciler watches `PhysicalDevice` filtered by `metadata.labels[blockstor.io/node]=self` (2026-05-10, scaffolded). `pkg/satellite/controllers/physicaldevice.go.PhysicalDeviceReconciler` lands the watch + label predicate, runs the Step-1 `DeviceMissing` pre-flight (no `DevicePath`/`CurrentDevPath` for a non-FILE kind → `Phase=Failed`), Step-4 `RequeueAfter:10s` when the target StoragePool CRD isn't here yet (race-matrix line 4), flips `Status.Phase=Attaching`, dispatches to the existing `satellite.Attach` pure function (covers Wipe + provider-specific create commands — all idempotent via the underlying `pvcreate` / `vgcreate` / `zpool create` short-circuits), registers the resulting provider via `Config.Apply.RegisterProvider`, and Deletes the CRD as completion signal. Wired into `controllers.NewManager`. Predicate pinned by 2 unit tests (own-node label match, foreign-node + unlabelled reject). Remaining for full closure: Step-2 pre-Attach `pvs grep` / `zpool status grep` explicit idempotency probe (today's idempotency relies on the create commands themselves), explicit `DeviceMissing` Condition (vs. the current `Phase=Failed` only).
...
- [~] CDP request creates `PhysicalDevice.Spec.AttachTo` referencing a `StoragePool` that doesn't exist (2026-05-10, partial). `PhysicalDeviceReconciler.targetPoolExists` lists `StoragePool` CRDs for this node and matches by `Spec.PoolName`; missing pool returns `ctrl.Result{RequeueAfter: 10s}` until the pool reconciler stamps the CRD. The bounded-retry → `Phase=Failed; condition: PoolMissing` transition is a follow-up — today the reconciler requeues indefinitely, which is fine for the common "operator applies pool + device together" race but not for the "operator never applies the pool at all" case.
...
- [x] When `kubectl delete storagepool <name>` runs with `propagationPolicy=Foreground`, satellite's `StoragePool` reconciler picks up the `DeletionTimestamp` and runs on-disk teardown (2026-05-10). New `storage.Provider.Destroy(ctx)` method on the interface; LVM (Thick + Thin) issue `vgremove --force <vg>`; ZFS issues `zpool destroy -f <pool>`; FILE / LOOPFILE remove the pool directory. All providers run a pool-level idempotency probe first (`vgs`, `zpool list`, dir-exists) so a re-run after a partial teardown finishes cleanly. The new `StoragePoolReconciler.handlePoolDelete` finalizer dance stamps `blockstor.io.blockstor.io/satellite-storagepool` on observation, runs Destroy on DeletionTimestamp when `Spec.DestroyOnDelete=true`, deregisters the in-memory provider, then strips the finalizer. Pinned by `TestThickDestroyRunsVgremove` + `TestThickDestroyIdempotentOnMissingVG`. (`wipefs -a` per-device is a follow-up — the current Destroy path relies on `vgremove --force`'s own signature-wipe and `zpool destroy -f`'s on-disk-wipe semantics, which suffice for the discovery loop's "is this a PV/VG/zpool" filter.)
- [x] If `Spec.DestroyOnDelete=false`, the pool's `StoragePool` CRD is removed but the underlying VG/zpool stays intact on disk (2026-05-10). `handlePoolDelete` only invokes Destroy when `Spec.DestroyOnDelete=true`; default `false` runs the in-memory provider deregister + finalizer strip but leaves on-disk state intact. Operators re-importing the data later can `vgchange -ay` / `zpool import` manually.
- [x] Discovery loop on next tick re-publishes the freed devices as new `PhysicalDevice` CRDs (2026-05-10). End-to-end flow now works: `StoragePool` Destroy wipes the on-disk VG/zpool via `vgremove --force` / `zpool destroy -f` (which also wipes the PV/zpool signature `lsblk` would surface), the satellite's existing discovery loop (`pkg/satellite/discovery.go`) walks `lsblk`, filters out devices still carrying PV/VG/zpool signatures, and `DiscoveryDiff` produces a `Create` action for any "now-free" device. The CRD name is derived from the device's stable-ID (`/dev/disk/by-id/wwn-*` → `scsi-SATA_*` → `nvme-*`), so re-attach via the same `linstor physical-storage create-device-pool` against the same WWN is deterministic.
