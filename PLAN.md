...
- [ ] `tests/e2e/tiebreaker.sh` flakes (~50% on back-to-back iters, 2026-05-12). Operational stuck-DRBD root cause fully addressed: PreStop hook on the satellite DaemonSet runs `drbdadm down` on every active resource before SIGTERM (commit `353da81`), and iter.sh switched the satellite-pod delete to graceful so PreStop actually runs. iter.sh also now strips finalizers before cleanup-delete (commit `0c81b0b`) so the cleanup step doesn't hang on the `blockstor.io.blockstor.io/satellite-resource` finalizer left behind by force-delete. Controller-side fixes also landed: NotFound tolerance on Get + Patch in the DRBD-IDs allocator (commits `5448199` + `f94d4b4`), 5s RequeueAfter on every RD reconcile to backstop cache-warmup races (commit `abb01b2`). What's left is a sub-second timing race: when the test polls every 1s and the reconciler creates the witness ~10-20ms after a poll, the next poll lands 980ms+ later — within the 60s deadline most of the time but not always. The deeper fix is to switch ensureTiebreaker's `ListByDefinition` from the cached client to APIReader-direct so the reconciler sees the just-applied second Resource on its first wake-up (no cache lag). Tracked under 8.8 follow-up.
