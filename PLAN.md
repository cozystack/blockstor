...
- [ ] Volume resize end-to-end with a real PVC: write checksum, grow via REST, verify checksum + filesystem sees the new size. Currently only the satellite-side resize chain is unit-tested. **Scaffold ready (2026-05-13)**: `tests/e2e/resize-pvc.sh` provisions a StorageClass with `allowVolumeExpansion: true` + ext4, claims a 128 MiB PVC, writes a 32 MiB md5-checksummed blob, patches the PVC to 256 MiB, asserts both `PVC.status.capacity == 256Mi` and that `df -k /data` reports a ≥1.5× grow (catches no-fs-resize false-success), then verifies the md5 round-tripped intact. Awaits a stand run to flip to [x].
