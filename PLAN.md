...
- [~] Delete `proto/satellite/v1alpha1/satellite.proto` + `pkg/satellite/proto/` (2026-05-11). `proto/satellite/v1alpha1/satellite.proto` + `pkg/satellite/proto/satellite_grpc.pb.go` (gRPC service stubs) are gone — no live gRPC consumers remain. `pkg/satellite/proto/satellite.pb.go` (pure message types) stays as the in-process wire format `Reconciler.Apply` + `dispatcher.BuildDesired` consume; a follow-up refactor will replace them with native Go structs.
