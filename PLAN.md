...
- **Stand deploy — image digest pinning** (post-mortem of silent Run 26-35 stale apiserver, 2026-05-19): `stand/build-images.sh` captures the per-image digest emitted by `docker push` into `.work/_factory/digest-<name>.txt`, and `stand/install-blockstor.sh` (via the new `stand/render-manifest.sh`) substitutes the floating `:dev` tag in each manifest with `@sha256:<digest>` before `kubectl apply`. Kubernetes treats digest references as immutable — every rebuild yields a different digest, every deploy is a real spec change and a real rolling update, no `imagePullPolicy: Always` + cache fight, no `kubectl rollout restart` dance. Bug 326 fix had landed and unit tests passed, but the live e2e2 apiserver pod was still running a pre-fix digest cached by containerd; all of Runs 26-35 silently tested against stale apiserver code. `iter.sh` now re-applies the manifests through the same helper so the hot dev loop picks up new digests on every iteration. Sanity check without a stand: `DRY_RUN=1 ./stand/install-blockstor.sh .work/foo | grep image:` must show `@sha256:` references (or fall back to `:dev` with a WARN on stderr when digest files are absent — fresh checkouts / local dev).
...
- [x] **Satellites register end-to-end** (2026-05-08):
      ...
      RPC creates a `Node.blockstor.cozystack.io` CRD per worker.
      `kubectl get nodes.blockstor.cozystack.io` shows 3/3 of them
...
- [x] Resource on 2 nodes replicates and goes UpToDate (real DRBD smoke) — **closed end-to-end** with auto-primary seed (2026-05-08): `kubectl apply RD + 2 Resource` → controller → satellite → file-backed LV (loop-attached via losetup) + drbdadm adjust + auto-primary --force → both peers `disk:UpToDate peer-disk:UpToDate` without any manual drbdadm. Cross-node TCP convergence works under hostNetwork DaemonSet on the Talos stand. **Bytes-perfect data replication confirmed**: 1 MiB random data written on worker-1 (Primary) reads back identical on worker-2 (Primary after failover). Automated regression: `make smoke-blockstor NAME=<cluster>` (`tests/smoke-blockstor.sh`) drives the full lifecycle (apply → UpToDate → write → failover read → md5 match → delete) and exits 0 on success.
      ...
      **Lifecycle complete**: finalizer-driven teardown via the new
      ...
      stand. `kubectl delete resource.blockstor.cozystack.io
...
5. `kubectl apply -f stand/blockstor-satellite-daemonset.yaml` —
   ...
   nodes.blockstor.cozystack.io` shows the 3 workers.
6. `curl POST /v1/resource-definitions` — 201, CRD created, `kubectl
   get resourcedefinitions.blockstor.cozystack.io` shows it; stats
...
- The `kventries.blockstor.cozystack.io` CRD type is gone. Cluster-
...
- [x] StoragePool CRD delete is deregister-only (2026-05-11, design corrected). The original PLAN proposed `Spec.DestroyOnDelete=true` running `vgremove --force` / `zpool destroy` from the satellite — that's wrong by design. StoragePool lifecycle is pure registration: deleting the CRD ONLY deregisters the in-memory provider and never touches the backend. On-disk pool creation is operator-driven via `linstor physical-storage create-device-pool`; on-disk teardown is an explicit out-of-band operator concern (blockstor refuses to `vgremove`/`zpool destroy` to avoid surprising data loss). `storage.Provider.Destroy` interface method + `Spec.DestroyOnDelete` field were both removed. `StoragePoolReconciler.handlePoolDelete` stamps `blockstor.cozystack.io/satellite-storagepool` on first observation, deregisters the provider on DeletionTimestamp, strips the finalizer. Pinned by `TestStoragePoolReconcileDeleteIsDeregisterOnly` (asserts no destructive command is ever issued).
...
- [x] Finalizers on `PhysicalDevice` (only set while `Spec.AttachTo` is non-nil) prevent kube-apiserver from removing the CRD before the satellite's teardown ran (2026-05-10). `PhysicalDeviceReconciler` stamps `blockstor.cozystack.io/physicaldevice-attach` as soon as it observes `Spec.AttachTo`, strips it in `runAttach` just before the delete-as-completion call, and shares one helper (`stripAttachFinalizer`) for both the "operator cleared AttachTo after a failed attempt" cleanup path and the "`kubectl delete physicaldevice X` mid-attach" honour-the-DeletionTimestamp path.
...
- After every state-mutating CLI call, poll Status for ≤30s via
  `kubectl get resources.blockstor.cozystack.io -o jsonpath=...` until expected state.
