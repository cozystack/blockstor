...
- [ ] Delete `proto/satellite/v1alpha1/satellite.proto` + `pkg/satellite/proto/`. Still required by the surviving observe / capacity gRPC paths; closes once the satellite writes Resource.Status directly via SSA.
- [~] Delete `pkg/satellite/grpc_server.go` + `pkg/satellitecontroller/` (2026-05-11). `pkg/satellite/grpc_server.go` + its tests deleted; the satellite no longer serves ApplyResources / DeleteResource / CreateSnapshot via gRPC. `pkg/satellitecontroller/` (the controller-side gRPC server that still receives ObserveStreamEvent + capacity push) stays until the satellite migrates to direct Status SSA.
- [x] Delete the controller-side dispatcher (2026-05-11). `pkg/dispatcher` is now pure CRD → `DesiredResource` translation (`BuildDesired` + helpers + `peerAddress`); the gRPC `Dispatcher` type, `Dialer` interface, `Apply` / `DeleteResource` / `CreateSnapshot` methods, the controller-side `ResourceReconciler.dispatchApply` / `runDelete`, and the `SnapshotReconciler` fan-out are gone. The controller's `ResourceReconciler` now does CRD housekeeping (DRBD-ID allocation, seed-from-Gi, auto-diskful promotion) only; satellite c-r path picks up the resulting Resource via watch and runs the apply chain. The legacy controller-side `blockstor.io.blockstor.io/resource` finalizer is stripped on observe (rolling-upgrade cleanup); the satellite's own `blockstor.io.blockstor.io/satellite-resource` finalizer owns teardown end-to-end.
- [~] Delete the gRPC supervise loops in `pkg/satellite/agent.go` (2026-05-11). `startGRPCServer` + `runCapacityLoop` + `capacityInterval` const are gone — capacity reporting moved to `pkg/satellite/controllers/storagepool.go.writeCapacity`, which fires on every Reconcile and re-schedules every 30 s via `RequeueAfter`. `runObserveLoop` / `superviseObserveLoop` / `dial` / `hello` stay because the satellite still pushes ObserveStreamEvent to the controller; closes once Status SSA replaces the observe path.
