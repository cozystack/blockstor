...
- [ ] `tests/e2e/tiebreaker.sh` flakes when run immediately after `two-volume-rd` in the same iter batch (2026-05-12). Root cause partially identified: `kubectl delete pod --force --grace-period=0` in iter's rollout step terminates the satellite before its finalizer runs `drbdadm down`, leaving DRBD-9 with a `Connecting` connection state pointing at the gone peer pod. Subsequent `drbdsetup down <res>` hangs forever waiting for that connection to gracefully tear. The DRBD kernel module is host-level (Talos node, shared across pod restarts), so the only way to fully clear stuck state without rebooting the node is `rmmod drbd && modprobe drbd` from the satellite container — needs CAP_SYS_MODULE on the Talos profile. iter.sh `drbdadm-down` step now has 30s outer + 5s per-call timeouts (commit `843c60c`) so iter doesn't hang on stuck state, but the underlying DRBD state still blocks the next scenario's create-md. Controller-side fixes already landed in `5448199` + `f94d4b4` so the race doesn't surface as a hard reconcile error. Follow-up: either teach the satellite to drain DRBD on SIGTERM (PreStop hook with `drbdadm down --all` + sleep), or thread `--force` through `drbdsetup down` once the kernel module supports it.
