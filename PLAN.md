...
- [~] If the target `StoragePool` CRD doesn't exist yet, controller creates it (2026-05-10). `handlePhysicalStorageCreate` now calls `ensureStoragePoolForAttach` before flipping `Spec.AttachTo` on the device: a `Get(node, pool)` lookup against the store, on `ErrNotFound` a `Create` with `Spec.NodeName`, `Spec.ProviderKind`, and a Props bag built from `with_storage_pool.props` (operator-supplied) with AttachTo-derived fallbacks. Pre-existing pools win via the `Get→short-circuit` path, so a parallel `kubectl apply` is non-destructive. Pinned by 2 new tests (auto-creates + preserves-existing). Remaining: OwnerReference wiring from PhysicalDevice → StoragePool so cascade-delete works (would require either store-level Spec.OwnerReferences plumbing or a CRD-direct path).
